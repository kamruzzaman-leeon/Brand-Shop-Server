...
async function run() {
    try {
        ...
        const BrandShopCollection = client.db('BrandShopDB').collection('product')

        //data create
        app.post('/product', async (req, res) => {
            const newProduct = req.body;
            console.log(newProduct);
            const result = await BrandShopCollection.insertOne(newProduct);
            res.send(result);
    ...
    app.get('/product', async (req, res) => {
        ...
        app.get('/product/:brand',async(req,res)=>{
            ...
            const result = await BrandShopCollection.find(query).toArray();
            res.send(result)
        })
